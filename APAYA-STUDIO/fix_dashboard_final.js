...
if(idx8 !== -1 && !lines.find(l => l.includes('triggerUpscaleGenerate'))) {
    const upscaleJs = `        function updateSwapBASlider(val) { document.getElementById('swap-img-after').style.clipPath = \`inset(0 0 0 \${val}%)\`; document.getElementById('swap-ba-handle').style.left = val + '%'; }
    ...
    lines.splice(idx8+1, 0, upscaleJs);
    console.log('Final JS Fixes applied');
}
